...
function addHighlight(keyIdToAddHightlight) {
  let elementToHighlight =
    window.document.getElementsByClassName(keyIdToAddHightlight);
...
class KeyBoardv2 extends Component {
  ...
  static getDerivedStateFromProps(nextProps, prevState) {
    if (nextProps.pressedKey === prevState.prevPressedKey) {
      return null;
    }

    if (nextProps.pressedKey === undefined) {
      return {
        prevPressedKey: undefined,
        keyIdToPress: null,
        keyboardLayerId: null,
      };
    }
    ...
    return {
      prevPressedKey: nextProps.pressedKey,
      keyIdToPress: keyId,
      keyboardLayerId: layerId,
    };
  }

  componentDidUpdate(prevProps, prevState) {
    if (prevProps.pressedKey !== this.props.pressedKey) {
      // remove previous highlight
      if (prevState.keyIdToPress) {
        removeHighlight(prevState.keyIdToPress);
      }

      // add new highlight
      if (this.state.keyIdToPress) {
        addHighlight(this.state.keyIdToPress);
      }

      // shift key logic
      if (this.state.keyboardLayerId) {
        addHighlight("c16"); // shift
      } else {
        removeHighlight("c16");
      }
    }
  ...
  render() {
    ...
    let renderedQwertyKeys = qwertyKeyIds.map((keyId, index) => {
      return (
        <li key={keyId}>
          <a className={"key " + keyId}>
            <span className="layer1">
              {keyMap[keyId][currentKeyboard][0][1]}
            </span>
            <span className="layer0">
              {keyMap[keyId][currentKeyboard][0][0]}
            </span>
            <span className="bg-eng">{keyMap[keyId].english}</span>
          </a>
        </li>
      );
    });
